    ...
    try {
      ...
      if (!hermesReady) {
          ...
          for (int attempt = 1; attempt <= maxCloneAttempts; attempt++) {
            try {
              onProgress(SetupState(
                ...
                progress: 0.0,
                message:
                    '正在克隆 SutaAgent（镜像 ${mi + 1}/${mirrors.length}，第 $attempt 次）...',
              ));
              _updateSetupNotification(
                '正在克隆 SutaAgent（镜像 ${mi + 1}/${mirrors.length}）...',
                progress: 70,
              );
              await prootRun(
        ...
        onProgress(const SetupState(
          step: SetupStep.installingHermesAgent,
          progress: 0.3,
          message: 'SutaAgent 已存在，跳过克隆',
        ));
        _updateSetupNotification('SutaAgent 已存在，跳过', progress: 75);
      }
